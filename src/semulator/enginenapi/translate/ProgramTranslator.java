...
import semulator.enginenapi.xml.gen.SInstructionArgument;
import semulator.enginenapi.xml.gen.SProgram;
import semulator.enginenapi.xml.gen.SInstruction;
...
import java.util.stream.Collectors;
    ...
    public final class ProgramTranslator {
        ...
        public static Result translate(SProgram sProgram) {
            ...
            List<SInstruction> code = new ArrayList<>();
        ...
        private static String trimToNull(String s) { return (s == null) ? null : (s.trim().isEmpty() ? null : s.trim()); }

        private static String errorMessageByLine(int line, String msg) {
            return STR."Error in line \{line}: \{msg}";
        }
        ...
        private static Variable newVarStrict(String name) {
            ...
            return new VariableImpl(name, variableType);
        }
        ...
        public static final class SyntheticPlaceholder implements Instruction {
            ...
            public String toDisplayString() {
                return switch (name) {
                    case "ASSIGNMENT" -> variable.getName() + " <- " + argsMap.getOrDefault("assignedVariable","?");
                    case "ZERO_VARIABLE" -> variable.getName() + " <- 0";
                    case "CONSTANT_ASSIGNMENT" -> variable.getName() + " <- " + argsMap.getOrDefault("constantValue","?");
                    case "GOTO_LABEL" -> "GOTO " + argsMap.getOrDefault("gotoLabel","?");
                    case "JUMP_ZERO" -> "JZ " + variable.getName() + " -> " + argsMap.getOrDefault("JZLabel","?");
                    case "JUMP_EQUAL_CONSTANT" ->
                            "JE " + variable.getName() + " == " + argsMap.getOrDefault("constantValue","?") +
                                    " -> " + argsMap.getOrDefault("JEConstantLabel","?");
                    case "JUMP_EQUAL_VARIABLE" ->
                            "JE " + variable.getName() + " == " + argsMap.getOrDefault("JEVariable","?") +
                                    " -> " + argsMap.getOrDefault("JEVariableLabel","?");
                    default -> name + "(" + (variable != null ? variable.getName() : "") + ")";
                };
            }
